...
def step_venv(args, data_dir: Path) -> Path:
    ...
    # air-gapped install.
    # GitHub's plain release-download URL is not a pip-resolvable
    # index; `/expanded_assets/<tag>` returns an HTML asset listing
    # that pip's `--find-links` understands.
    bridge_url = (os.environ.get("DEJAVU_BRIDGE_INDEX_URL")
                  or "https://github.com/HthSolid/claude-dejavu/"
                     "releases/expanded_assets/dejavu-bridge-0.1.0")
